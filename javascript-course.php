...
$topic = "JavaScript"; 
$page_title = "JavaScript Fundamentals"; 
$theme_color = "#d97706"; 
$theme_bg = "linear-gradient(135deg, #fef3c7 0%, #fde68a 100%)"; 
$msg = "";
...
<head>
    ...
    <style>
        .course-header { text-align: center; padding: 2.5rem 1rem; background: <?php echo $theme_bg; ?>; border-radius: 12px; margin-bottom: 2rem; color: #1f2937; }
        .course-content { background: #fff; padding: 2.5rem; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); border: 1px solid #e5e7eb; margin-bottom: 2rem; }
        .video-container { position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; max-width: 100%; border-radius: 8px; margin-bottom: 1.5rem; border: 1px solid #e5e7eb;}
        .video-container iframe { position: absolute; top: 0; left: 0; width: 100%; height: 100%; }
        h2 { border-bottom: 2px solid #e5e7eb; padding-bottom: 0.5rem; margin-bottom: 1.5rem; color: #111827; }
        .quiz-question { background: #f9fafb; padding: 1.5rem; border-radius: 8px; margin-bottom: 1.5rem; border-left: 4px solid <?php echo $theme_color; ?>; }
        .quiz-question p { margin-bottom: 1rem; font-size: 1.1rem; color: #1f2937; font-weight:600;}
...
<body style="background-color: #f3f4f6;">
    ...
    <div class="container wide" style="margin-top:2rem;">
        ...
        <?php if($lessons_res && pg_num_rows($lessons_res) > 0): ?>
            ...
            <?php $lesson_num = 1; while($lesson = pg_fetch_assoc($lessons_res)): ?>
            <div class="course-content">
                <h2>Lesson <?php echo $lesson_num++; ?>: <?php echo htmlspecialchars($lesson['title']); ?></h2>
                
                <?php if(!empty($lesson['video_link'])): ?>
                <?php if (strpos($lesson['video_link'], 'youtube.com') !== false): ?>
                <div class="video-container">
                    <iframe src="<?php echo htmlspecialchars($lesson['video_link']); ?>" frameborder="0" allowfullscreen></iframe>
                </div>
                <?php else: ?>
                <div class="video-container" style="padding-bottom: 0; height: auto;">
                    <video width="100%" controls style="border-radius: 8px;">
                        <source src="<?php echo htmlspecialchars($lesson['video_link']); ?>" type="video/mp4">
        ...
        <?php if($quizzes_res && pg_num_rows($quizzes_res) > 0): ?>
            ...
            <div class="course-content" style="border-top: 4px solid #059669;">
                <form method="POST">
                    <?php $q_num = 1; while($q = pg_fetch_assoc($quizzes_res)): ?>
